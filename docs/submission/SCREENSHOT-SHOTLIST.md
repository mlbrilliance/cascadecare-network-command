...
| 4 | `04-case-app.png` | The **OOTB Case App** for a case instance: caseSummary + timeline + SLA tiles + stakeholder/obligation sections rendering on **1.0.35**. | "The out-of-the-box UiPath Case App: live case timeline, SLA tiles, and stakeholder/obligation sections." | C2 Platform Usage · C4 Variety | Screenshot **4** (`:211`) — clears `[VERIFY: 1.0.35]` |
...
- **OOTB Case App renders on 1.0.35** (`DEVPOST.md:80`, `:211`): open the Case App for a live
  instance and confirm caseSummary + sections render → if yes, change `[VERIFY: 1.0.35]` /
  `[VERIFY: 1.0.35 deployed + renders]` to a plain claim; if not, *delete the claim* rather than
